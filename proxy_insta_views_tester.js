       ...
       console.log(last_date_updated)
        if(!last_date_updated.includes(current_date_string)){
            ...
            if(views_object != null){
                ...
                index_stopped = i
                finished_all = false
                console.log('We did not find the reels tab, check screenshot in proxy_insta_views_tester.png')
...
const get_insta_views = async (video_url, profile_url) => {
    ...
    const args = [
        ...
        '--ignore-certifcate-errors-spki-list',
             '--proxy-server=ca.smartproxy.com:20000'          ]
    ...
    await page.screenshot({path: 'proxy_insta_views_tester.png'});

     // await page.waitForXPath('/html/body/div[1]/section/main/div/div[1]/a[2]')
     let exit_login_prompt = await page.$x('/html/body/div[1]/section/nav/div[2]/div/div/div[3]/div/div/div/button');
     // console.log(exit_login_prompt)
    if(exit_login_prompt[0] != null){
        await exit_login_prompt[0].click();
    }
...
const getXPath = async(page, video_url, iteration_counter) => {
    ...
    let found_the_vid = false

    //Inside a while loop so that if we haven't found the video
    // then we need to scroll just a bit down
    let vid_num = 0
    ...
    let row_num = 1
    if(vid_num % 4 == 0){
        row_num = Math.floor(vid_num/4)
    }
    else{
        row_num = Math.floor(vid_num/4) + 1 
    }
    ...
    //Case where we have three tabs
        ...
        // let sample_XPath_8th_vid =     '/html/body/div[1]/section/main/div/div[3]/div/div/div/div[2]/div[4]/div/a/div[2]/div[2]/div/div/div/div[2]/span'
        //                                '/html/body/div[1]/section/main/div/div[3]/div/div/div/div[1]/div[4]/div/a/div[2]/div[2]/div/div/div/div[2]/span'
...
async function autoScroll(page){
    await page.evaluate(async () => {
        let finishTime = new Date().getTime() + 10;
...
// get_insta_views("https://www.instagram.com/reel/CRRnoGsFhAY/", "https://www.instagram.com/jeanca.milo/")
